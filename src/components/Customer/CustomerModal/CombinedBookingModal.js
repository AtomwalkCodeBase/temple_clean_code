...
const VariationsSectionTitle = styled.h3`
  font-weight: 800;
  color: #1e293b;
  font-size: 1.2rem;
  margin-bottom: 1rem;
...
function timeToMinutes(timeStr) {
  ...
  const [h, m, s] = timeStr.split(":").map((n) => Number.parseInt(n, 10));
...
export default function CombinedBookingModal({
  ...
  const [selectedKey, setSelectedKey] = useState(() => {
    if (preSelectedDate) {
      // Validate that pre-selected date is not in the past
      const [year, month, day] = preSelectedDate.split("-");
      const preDate = new Date(year, month - 1, day);
      if (preDate < today) return null;
      ...
      if (selectedKey) {
        ...
        try {
          const bookings = await getAdminBookingList({
            booking_date: toAPIDate(selectedKey),
          });
  ...
  const startWeekday = firstDayOfMonth(viewDate).getDay();
  ...
  const isVariationBooked = (variation) => {
    ...
    if (service_type === "EVENT") {
      ...
      return totalBookedQty + variation.max_participant >= capacity;
    } else {
  ...
  const getBookedCountForVariation = (variation) => {
    ...
    return bookingList.filter((booking) => {
      ...
      const isSameVariation =
        booking.service_variation_data?.id === variation.id;
      return isSameService && isSameVariation;
    }).length;
  };
  ...
  const handleConfirm = () => {
    if (selectedKey && selectedVariation !== null) {
      onConfirm(selectedKey, selectedVariation);
    }
  };
      ...
      {open && (
        ...
        >
          ...
          >
            <Header>
              <div className="title">
                <FiCalendar size={20} /> Book Your Service
              </div>
            ...
            <Body>
              ...
              <VariationsSection>
                ...
                {!selectedKey ? (
                  ...
                  <LoadingMessage>Checking availability...</LoadingMessage>
                ) : variations.length === 0 ? (
                  <LoadingMessage>
                    No variations available for this date
                  </LoadingMessage>
                ) : (
                  <VariationsContainer>
                    {variations.map((v, idx) => {
                      ...
                      return (
                        <VariationCard
                          key={idx}
                          disabled={isDisabled}
                          onClick={() => !isDisabled && setSelectedVariation(v)}
                        ...
                        >
                          ...
                          <div className="price">
                            ...
                            {(
                              ...
                              (Number.parseFloat(
                                v.pricing_rule_data?.week_day_price
                              ) || 0) +
                              (Number.parseFloat(
                                v.pricing_rule_data?.time_price
                              ) || 0) +
                              (Number.parseFloat(
                                v.pricing_rule_data?.date_price
                              ) || 0)
                            ).toLocaleString()}
                          </div>
                        </VariationCard>
            ...
            <Footer>
              ...
              <ConfirmButton
                disabled={!selectedKey || selectedVariation === null}
